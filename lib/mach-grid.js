...
var MachGrid = (function(){
			...
			if(e.target){
				...
				while(finger != htmlElement){
					if(_useStandardMatchs){
						...
						shouldHandle = finger.matchesSelector(selector, htmlElement);
					}
					if(shouldHandle){
	...
	MachGrid.prototype._initEventHandlers = function(){
		...
		this.table.addEventListener("wheel", (function(e){
			var amount = Math.floor(e.wheelDeltaY / this.protoRow.offsetHeight) * -1;
		...
		addDelegate(this.table, "tr", "click", function(e){
			me.selectedIndex = parseInt(this.getAttribute("data-row-index"));
		});
	};
